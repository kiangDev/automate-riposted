...
def handle_customer_capture_postal_code_alert(window, timeout=4):
    ...
    แล้วลดลงเหลือ 2 ไปรอบก่อน เพราะกลัวเสียเวลาเปล่าตอนไม่เจอ Alert (ฟังก์ชัน
    นี้ถูกเรียกใน main() สูงสุด 3 รอบต่อแถว) -- แต่ 2 วิสั้นเกินไปจริง ผู้ใช้
    ทดสอบแล้วพบว่า Alert นี้เด้งช้ากว่า 2 วิ ทำให้เช็คไม่ทัน พลาด Alert ไปเลย
    (เท่ากับ "ยกเลิก" ไม่ได้กด รหัสไปรษณีย์ที่พิมพ์เองอาจถูกเปลี่ยนเป็นค่าที่
    ระบบแนะนำแทนโดยไม่ตั้งใจ) -- ปรับขึ้นเป็น 4 วิ เป็นจุดกึ่งกลาง

    หมายเหตุ: ในทางปฏิบัติจริงฟังก์ชันนี้มักถูกเรียกแค่ "รอบเดียว" ต่อแถว
    ไม่ใช่ครบ 3 รอบเสมอ เพราะลูปใน main() จะ break ทันทีที่ถึงหน้าค้นหา
    ที่อยู่แล้ว (ดูคอมเมนต์ ADDRESS_SEARCH_INPUT_AUTO_ID) ต้นทุนจริงๆ ต่อแถว
    ส่วนใหญ่จึงเป็นแค่ ~4 วิ ไม่ใช่ 4x3=12 วิ
    """
    if is_control_visible(
        window,
        timeout=timeout,
...
def main():
    ...
    try:
        ...
        # แก้: ผู้ใช้ส่ง log จริงมาแล้วเจอซ้ำ 2 รอบว่าแถวแรกสุดของการรันแต่ละ
        # ครั้งมักจะ TimeoutError ตอนหา "Shipping" (HOME_AUTO_ID) ทันที --
        # สาเหตุน่าจะเป็นเพราะแอปอาจไม่ได้อยู่หน้าเมนูหลักพอดีตอนสคริปต์เพิ่ง
        # เชื่อมต่อ (เช่น ค้างอยู่หน้าอื่นจากรอบทดสอบก่อนหน้า) ของเดิมสมมติ
        # เฉยๆ ว่าต้องเห็น "Shipping" อยู่แล้ว ไม่เคยเช็ค/กู้คืนก่อนเริ่มแถว
        # แรกเลย เรียก recover_ui() ล่วงหน้าหนึ่งครั้งตรงนี้ เพื่อบังคับกลับสู่
        # หน้าเมนูหลักที่มี "Shipping" ให้แน่ใจก่อนเริ่มแถวแรกจริงๆ
        print("[DEBUG] กำลังปรับหน้าจอให้อยู่ที่เมนูหลักก่อนเริ่มแถวแรก...")
            ...
            with open(LOG_FILENAME, mode="a", encoding="utf-8-sig") as log_file:
                ...
                for index, row in enumerate(rows, start=1):
                    ...
                    try:
                        ...
                        #
                        # แก้: ผู้ใช้ส่ง traceback จริงมาแล้วเจอ 10 วิยังไม่พอ
                        # อีก (pywinauto.timings.TimeoutError: timed out ตรง
                        # control.wait() บรรทัดนี้เป๊ะ) แต่ที่สำคัญคือ
                        # controls_fail_ShippingService2572.txt ที่ dump
                        # อัตโนมัติตอน error (แทบจะทันทีหลัง timeout หมด) กลับ
                        # เจอปุ่ม auto_id="ShippingService_2572" อยู่ปกติ มี
                        # พิกัดสมเหตุสมผล (ไม่ใช่ 0x0/ไม่ใช่หน้าอื่น) พร้อมปุ่ม
                        # บริการอื่นอีก ~39 ปุ่มโหลดครบหมดแล้ว -- แปลว่าหน้า
                        # โหลดเสร็จช้ากว่า 10 วิไปเพียงเสี้ยววินาที (โหลดเสร็จ
                        # "หลัง" wait() ยอมแพ้ไปแล้วพอดี ไม่ใช่ปุ่มหาย/หน้าผิด)
                        # นี่คือ pattern "รอของที่ควรมีอยู่แล้วแค่โหลดช้า" ไม่ใช่
                        # pattern เช็ค Alert ที่มักไม่มี -- เพิ่ม timeout ตรงนี้
                        # ไม่มีต้นทุนเปล่าตอนโหลดเร็ว (happy path คลิกได้ทันที
                        # ที่ปุ่มโผล่ ไม่ต้องรอครบเพดาน) ปลอดภัยที่จะเพิ่มเป็น
                        # 20 วิ ให้มีระยะเผื่อมากขึ้น
                        wait_and_click(
                            main_window,
                            auto_id=SHIPPING_SERVICE_AUTO_ID,
                            control_type="Button",
